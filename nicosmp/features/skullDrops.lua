...
local function format_enum_name(input)
    local words = {}
    for word in input:lower():gmatch("[^_]+") do
        table.insert(words, word:sub(1, 1):upper()..word:sub(2))
    end
    return table.concat(words, " ")
end

addEvent(EntityDeathEvent, function(event)
    local source = event.getDamageSource()
    if source == nil then return end
    local attacker = source.getCausingEntity()
    ...
    if victimType == "PLAYER" then
        if not victim.isOnline() then return end

        local item = bukkit.buildItem("PLAYER_HEAD")
            :custom(function(meta)
                meta.setOwningPlayer(victim)
            end)
            :displayName("§eKopf von "..victim.getName())
            :lore({
                "§7Killer: §f"..attacker.getName()
            })
            :build()

        local loc = victim.getLocation()
        loc.getWorld().dropItemNaturally(loc, item)
        return
    end

    local data = MOB_TEXTURES[victimType]
    if data == nil then
        print("§emob heads: no data for "..victimType)
        return
    end

    if not random:chance(MOB_CHANCE) then
        if  victimType ~= "WITHER"
        and victimType ~= "ENDER_DRAGON"
        and victimType ~= "ELDER_GUARDIAN" then
            return
        end
    end
    ...
    if type(data) == "string" then
        item = bukkit.buildItem("PLAYER_HEAD")
            :skull_texture(data)
    else
        item = bukkit.buildItem(data.material)
    end


    item = item
        :displayName("§e"..format_enum_name(victimType).." Kopf")
        :lore({
            "§7Killer: §f"..attacker.getName()
        })
        :build()
